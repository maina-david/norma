    ...
    >
      @if(isset($bulk))
        <span class="text-left block px-4">
          {{ $inNorma ? __('compilation.context_question.inclusion.remove_from_stream') : __('compilation.context_question.inclusion.add_to_stream')  }}
        </span>
      @else
        <span class="text-center block px-4">
          {{ $inNorma ? __('interface.yes') : __('interface.no') }}
        </span>
      @endif
    </a>
  </x-slot:trigger>
    ...
    <div>
      ...
      <div>
        <textarea
          ...
          required
          rows="8"
          class="px-3 py-2 border leading-normal rounded-md shadow-sm border-norma-gray-300 focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50 block mt-1 w-full"
      ...
      <div class="border-t border-norma-gray-200 mt-4 pt-4 flex items-center justify-end space-x-4">

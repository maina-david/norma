@php
  ...
  $scrollOnLoad = $scrollTo ? "window.setTimeout(function() { window.scrollToItemWhenAvailable('#{$scrollTo}', '.norma-legislation', 'smooth'); }, 200)" : '';
@endphp
<div x-init="{{ $scrollOnLoad }}"></div>
@if ($work && !empty($work->source->source_content ?? '') && '<p>&nbsp;</p>' !== ($work->source->source_content ?? ''))
  <div class="italic text-norma-gray-600 px-5 py-3 text-xs">

...
<table class="w-full sm:-m-6">
  ...
  <tr class="border-b">
    ...
    <x-ui.td>
      <span class="text-norma-gray-500">{{ RiskRating::fromValue($response->assessmentItem->risk_rating)->label() }}</span>
    </x-ui.td>
  </tr>
  ...
  <tr class="border-b">
    ...
    <x-ui.td>
      @if($response->answered_at)
        <span class="text-norma-gray-500">{{ $response->lastAnsweredBy->full_name ?? '-' }},</span>
        <span class="text-norma-gray-500">{{ $response->answered_at?->format('d M Y') ?? '' }}</span>
      @else
        -
      @endif
    </x-ui.td>
  ...
  <tr class="border-b">
    ...
    <x-ui.td>
      <span class="text-norma-gray-500">{{ $response->assessmentItem->author->full_name ?? '-' }}</span>
    </x-ui.td>
  </tr>
</table>

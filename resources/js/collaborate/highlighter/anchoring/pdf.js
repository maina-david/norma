...
/* eslint-disable no-plusplus, no-param-reassign, no-await-in-loop */
 ...
 *
 * @typedef {import('./api').PDFPageView} PDFPageView
 * @typedef {import('./api').PDFViewer} PDFViewer
 */
...
async function getPageTextContent(pageIndex) {
  ...
  const getTextContent = async (pageIdx) => {
    const pageView = await getPageView(pageIdx);
...
function findInPages(pageIndexes, quoteSelector, positionHint) {
  ...
  const attempt = ([ctx, off]) => {
    const root = document.createElement('div');
    root.textContent = ctx;
    const anc = TextQuoteAnchor.fromSelector(root, quoteSelector);
    if (positionHint) {
      let hint = positionHint.start - off;
      hint = Math.max(0, hint);
      hint = Math.min(hint, ctx.length);
      return anc.toPositionAnchor({ hint });
    }
    return anc.toPositionAnchor();
  };

  const next = () => findInPages(rest, quoteSelector, positionHint);

  const cacheAndFinish = (anc) => {
    if (positionHint) {
    ...
    return anchorByPosition(pageIndex, anc.start, anc.end);
  };
  ...
  if (quote) {
    result = result.catch(() => {
      if (
        ...
        const { pageIndex, anc } = quotePositionCache[quote.exact][position.start];
        return anchorByPosition(pageIndex, anc.start, anc.end);
      }

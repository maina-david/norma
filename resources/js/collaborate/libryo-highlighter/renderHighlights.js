/**
 * Adapted from https://github.com/hypothesis/client/tree/master/src/annotator/highlighter.js
 */
...
function drawHighlightsAbovePdfCanvas(highlightEl) {
  ...
  const isCssBlendSupported = isCSSPropertySupported('mix-blend-mode', 'multiply');
 ...
 * @param {Object} options - The custom element to use to wrap all text nodes in
 * @return {HTMLElement[]} - Elements wrapping text in `normedRange` to add a highlight effect
 */
export const highlightRange = (normedRange, highlightId, options) => {
  if (!options) {
  ...
  const isPlaceholder =
    textNodes.length > 0 && closest(textNodes[0].parentNode, '.highlighter-placeholder') !== null;
  ...
  const highlights = [];
  textNodeSpans.forEach((nodes, count) => {
    // A custom element name is used here rather than `<span>` to reduce the
    // likelihood of highlights being hidden by page styling.
    const highlightEl = document.createElement(options.customElement);
    highlightEl.classList.add(options.highlightClass);

    if (options.cssClasses.length > 0) {
      highlightEl.classList.add(...options.cssClasses);
    }
    if (count === 0) {
      highlightEl.classList.add(options.firstClass);
    }
    if (count === textNodeSpans.length - 1) {
      highlightEl.classList.add(options.lastClass);
    }
    options.customAttributes.forEach(attr => {
      highlightEl.setAttribute(attr.key, attr.value);
    });

    highlightEl.setAttribute(options.idAttributeName, highlightId);
 ...
 */
export const removeHighlights = (highlights) => {
 ...
 * @return {Rect}
 */
export const getBoundingClientRect = (collection) => {

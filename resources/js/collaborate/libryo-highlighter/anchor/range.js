/**
 * Adapted from https://github.com/openannotation/annotator/blob/v1.2.x/src/range.coffee
 */
...
Range.nodeFromXPath = (xpath, root = document) => {
  const evaluateXPath = (xp, nsResolver = null) => {
    try {
...
Range.NormalizedRange = class NormalizedRange {
  ...
  limit(bounds) {
    const nodes = this.textNodes().filter((node) => {
      return node.parentNode.isSameNode(bounds) || bounds.contains(node.parentNode);
    });
  ...
  //
  // Returns an instance of SerializedRange.
  serialize(root, ignoreClasses) {
    const serialization = (node, isEnd) => {
      let origParent;
      if (ignoreClasses) {
        origParent = node.parentNode;

        const containsClass = (n) => {
          return ignoreClasses.some((cl) => {
            return n.classList.contains(cl);
          });
        };

        while (containsClass(origParent)) {
          origParent = origParent.parentNode;
        }
      } else {
        origParent = node.parentNode;
      }

      const xpath = Util.xpathFromNode(origParent, root);
      const textNodes = Util.getTextNodes(origParent);

      // Calculate real offset as the combined length of all the
      // preceding textNode siblings. We include the length of the
      // node if it's the end node.
      const ind = textNodes.findIndex((n) => n.isSameNode(node));
      const nodes = textNodes.slice(0, ind);
      let offset = 0;
  ...
  textNodes() {
    ...
    const start = textNodes.findIndex((n) => n.isSameNode(this.start));
    const end = textNodes.findIndex((n) => n.isSameNode(this.end));
...
Range.SerializedRange = class SerializedRange {
  ...
  normalize(root) {
    ...
    ['start', 'end'].forEach((p) => {
      ...
      if (range[p + 'Offset'] == null) {
        throw new Range.RangeError(
          `${p}offset`,
          `Couldn't find ${p} offset ${this[p + 'Offset']} in element ${this[p]}`,
        );
      }
    });
    ...
    rr.setStartBefore(range.startContainer);
    rr.setEndAfter(range.endContainer);
    range.commonAncestorContainer = rr.commonAncestorContainer;
  ...
  //
  // root           - A root Element from which to anchor the serialisation.
  // ignoreClasses - An array of classes that should be ignored.
  //
  // Returns an instance of SerializedRange.
  serialize(root, ignoreClasses) {
    return this.normalize(root).serialize(root, ignoreClasses);
  }

...
function mapForWork(fetched) {
  const grouped = {};

  fetched.forEach((item) => {
    const groupKey = item.work_title;
    ...
    grouped[groupKey].references_count += 1;
  });
  ...
  Object.keys(grouped).forEach((key) => {
    ...
    sorted.push({ ...grouped[key], children, assignees: Object.values(grouped[key].assignees) });
  });
...
function fetchItems() {
  ...
  axios.get('/actions/planner/references', { params })
    .then(({ data }) => data)
    .then(({ data }) => mapForWork([...data]))
    .finally(() => {
      loading.value = false;
    });
}

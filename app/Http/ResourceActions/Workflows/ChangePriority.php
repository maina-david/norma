...
class ChangePriority implements ResourceAction
{
    ...
    {
        return 'partials.workflows.collaborate.task.actions.change-priority';
    }
    ...
    public function handle(Request $request, array $resourceIds): RedirectResponse
    {
        ...
        Task::whereKey($resourceIds)
            ->get()
            ->each(function (Task $task) use ($priority) {
                $task->priority = $priority;
                $task->save();
            });

        Session::flash('flash.message', __('workflows.task.successfully_changed_priority'));

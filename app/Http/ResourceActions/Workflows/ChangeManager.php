...
class ChangeManager implements ResourceAction
{
    ...
    {
        /** @var string */
        return __('workflows.task.change_manager');
    }
    ...
    {
        return 'partials.workflows.collaborate.task.actions.change-manager';
    }
    ...
    public function handle(Request $request, array $resourceIds): RedirectResponse
    {
        ...
        Task::whereKey($resourceIds)
            ->get()
            ->each(function (Task $task) use ($manager) {
                $task->manager_id = $manager;
                $task->save();
            });

        Session::flash('flash.message', __('workflows.task.successfully_changed_manager'));

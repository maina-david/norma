...
class ApplyDraftSummaries implements ResourceAction
{
    /**
     * @param int $workId
    ...
    {
        return $user && $user->can('collaborate.requirements.summary.draft.apply');
    }
    ...
    {
        /** @var string */
        return __('corpus.reference.apply_summary_drafts');
    }
        ...
        Reference::where('work_id', $this->workId)
            ...
            ->chunk(2000, fn ($references) => $references->each(function ($ref) {
                if (!$ref->summary) {
                    $ref->summary()->create([
                        'reference_id' => $ref->id,
                        'summary_body' => $ref->summaryDraft?->summary_body,
                    ]);
                } else {
                    $ref->summary->update([
                        'summary_body' => $ref->summaryDraft?->summary_body,

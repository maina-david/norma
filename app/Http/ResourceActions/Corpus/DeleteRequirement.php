...
class DeleteRequirement implements ResourceAction
{
    /**
     * @param int $workId
    ...
    {
        return $user && $user->can('collaborate.corpus.reference.requirement.delete-applied');
    }
    ...
    {
        /** @var string */
        return __('corpus.reference.delete_requirement');
    }
    ...
    public function trigger(Request $request, array $resourceIds): RedirectResponse
    {
        ...
        ReferenceRequirement::whereRelation('reference', fn ($q) => $q->where('work_id', $this->workId)->typeCitation())

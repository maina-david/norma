...
use App\Events\Collaborators\CollaboratorActivated;
...
class ActivateCollaborator implements ResourceAction
{
    /**
    ...
    {
        return $user && $user->can('collaborate.collaborators.collaborator.activate');
    }
    ...
    public function trigger(Request $request, array $resourceIds): RedirectResponse
    {
        Collaborator::whereKey($resourceIds)
            ...
            ->each(function (Collaborator $collaborator) {
                ...
                $collaborator->save();
                CollaboratorActivated::dispatch($collaborator);
            });

        Session::flash('flash.message', __('collaborators.successfully_activated'));

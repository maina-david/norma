...
class NormaRequest extends FormRequest
{
    ...
    {
        $rules = [
            'title' => ['string', 'required', 'max:255', Rule::unique((new Norma())->getTable())->where(function ($query) {
        ...
        if (!$this->activeOrganisationManager->isSingleOrgMode() || userCanManageAllOrgs()) {
            ...
            $rules['place_type_id'] = ['integer', 'nullable', Rule::exists(NormaType::class, 'id')];
        ...
        $rules['title'] = ['string', 'required', 'max:255', Rule::unique((new Norma())->getTable())->where(function ($query) {
            // @phpstan-ignore-next-line
            $normaId = $this->route()->parameter('norma');

            return $query->where('organisation_id', $this->organisation_id)->where('id', '!=', $normaId);
        })];

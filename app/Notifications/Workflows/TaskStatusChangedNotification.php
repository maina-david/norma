...
use App\Mail\Workflows\TaskStatusChangedEmail;
...
class TaskStatusChangedNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        ...
        return (new TaskStatusChangedEmail($this->task))->to($notifiable->email);
    }
    ...
    {
        return [
            __('notifications.task_status_changed'),
            '#' . $this->task->id . ' ' . ($this->task->title ?? ''),
            __('notifications.new_status') . ': ' . __('workflows.task.task_status.' . $this->task->task_status),
        ];
    }
}
